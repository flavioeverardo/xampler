...
Note that projected model counting on 2-QBFs is proven to be $\cntc\coNP$-complete~\cite{DurandHermannKolaitis05}. Further, we also suspect that both problems of counting all the subset-minimal vertex cover as well as counting subset-minimal independent dominating sets are hard for this complexity class. At least there are no known polynomial encodings for \SAT that precisely capture the solutions to these problems. Hence, it is unlikely that one can easily approximate the number of solutions using approximate SAT counting.
...
As part of the setup of the experiments and for comparison, we asked \xorro{} to estimate the count also by calculating the median, taken from the original ApproxMC Algorithm in~\cite{ChakrabortyMV13}.
...
\begin{table*}[t]
  ...
  \caption{Approximate answer set count over random instances of the Subset-Minimal Independent Dominating Sets problem.}\label{table:smds}
\end{table*}
...
%
On the other hand, for the subset-minimal vertex covers, we see that both counts were almost exactly on average. In this example, also the worst cases are close to an exact count.
%
