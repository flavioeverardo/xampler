...
model of~$\Pi^I$. %
We denoted the set of all answer sets of program~$\Pi$ by \sol. % of the program \fff.
%
...
%\end{itemize}
%
Let $n$, $m$ and $k$ be positive integers, we use $H(n, m, k)$ to denote a family of $k$-wise independent hash functions mapping $\{0, 1\}^n$ to $\{0, 1\}^n$.
...
We use an already evolved algorithm from the case of propositional satisfiability~\cite{ChakrabortyMV13} and lift it to ASP.
%
For our work we use a specific family of hash functions denoted by $H_{xor}(n, m, 3)$, 
%it randomly choosing bits from $y \in \{0, 1\}^n$, and xor them. 
%
%$H_{xor}(n, m, 3)$ 
to partition the set of models of an input formula into ``small" cells.
%
This family of hash functions has been used in \cite{gomes2006model,ChakrabortyMV13}, and is shown to be 3-wise independent in \cite{gomes2007near}.
%
Please refer to \cite{gomes2006model,gomes2007near,ChakrabortyMV13} for details.
