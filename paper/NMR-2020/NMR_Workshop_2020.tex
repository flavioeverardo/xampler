...
%
%In order to test whether the generated cells are indeed small, we choose a random cell and check if it is non-empty and has no more than pivot elements, where pivot is a threshold that depends only on the tolerance bound \ep. 
%%
...
%
Then it checks if the input program \fff has at least pivot number of solutions.
%
It uses the \xampler\ to check if the input program has at least $b \, (b = pivots+ 1)$ solutions.
%
If the total number of solutions $S$ of \fff are less than or equals to $b$ the algorithm return the answer set count $|S|$. 
%
...
%
The resulted non-$\bot$ estimates of the ASP model count $c$ returned by \countas are appended to the list $C$. 
%
