...
%
\title{Extending \xorro{} with Approximate Model Counting\thanks{The work has been supported by the Austrian Science Fund
    (FWF), Grants Y698 and P32830, 
and the Vienna Science and Technology Fund, Grant WWTF ICT19-065. It is also accepted for presentation at the NMR'20 workshop~\protect\cite{EverardoHecherShukla20}.}}
%
\titlerunning{Extending \xorro{}  with Approximate Model Counting}  % abbreviated title (for running head)
...
\institute{
  ...
  Tecnol\'ogico de Monterrey Puebla Campus, Mexico \and
...
only a few attention has been paid to treat parity constraints as well as reasoning modes like sampling or approximate model counting for ASP. %~\cite{kalepesc16a}.
%
...
%
%Rather than exact counting

%One approach is to count all the solutions  by enumeration, known as exact counting.
%
...
Intuitively a family $\mathcal{H} = {h : U \to [m]}$ of hash functions is $k$-wise independent if for any distinct keys $x_1, ..., x_k \in U$, the hash codes $h(x_1), ..., h(x_k)$ are independent random variables and for any fixed $x$, $h(x)$ is uniformly distributed in $[m]$.
%\end{itemize}
...
%	$S \= \xorro(\fff, pivot + 1)$ // {Solve pivot+1 AS}  \;
%	\eIf{ $|S| \leq pivot$}
%	{ % Open If
%		\textbf{return} $|S|$
%	} % Close If
...
\begin{algorithm}[t]
	/* Assume $z_1 , ..., z_n $ are the atoms of \fff */ \\
...
%	$ i \= l - 1$ \\
	...
	\While{($1 \leq |S| \leq pivot $) $\lor (i = n)$}{
		...
		$\alpha \gets \{0,1\}^{i-l}$ \\
		$S = \xorro(\fff \land (h(z_1, ..., z_n)=\alpha, pivot + 1)$ %\Comment*[r]{Solve $P \land XOR$}
	}
...
%
Then it conjuncts the chosen XOR with the program \fff and uses \xorro\ to check whether it has at most $pivot +1$ models.
% 
